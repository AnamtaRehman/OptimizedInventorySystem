...
<?php
  if(isset($_POST['add_item'])){
   ...
   $req_fields = array( 'name', 'name_as_per_store','type','sub_type','make','model_number','oem','internet_connectivity','wifi','bluetooth','mac_address','network_port','configuration','processor','ram_type','ram_size','ssd_size','hd_size','windows','oem_warranty','sr_no_by_rac','remarks');
   ...
   if(empty($errors)){
       ...
       $sr_no_by_rac= remove_junk($db->escape($_POST['sr_no_by_rac']));
       $remarks= remove_junk($db->escape($_POST['remarks']));
        $query = "INSERT INTO add_items (";
        $query .="name, name_as_per_store,type,sub_type,make,model_number,oem,internet_connectivity,wifi,bluetooth,mac_address,network_port,configuration,processor,ram_type,ram_size,ssd_size,hd_size,windows,oem_warranty,sr_no_by_rac,remarks";
        $query .=") VALUES (";
        $query .=" '{$name}', '{$name_as_per_store}', '{$type}','{$sub_type}','{$make}','{$model_number}','{$oem}','{$internet_connectivity}','{$wifi}','{$bluetooth}', '{$mac_address}', '{$network_port}','{$configuration}','{$processor}','{$ram_type}','{$ram_size}','{$ssd_size}','{$hd_size}','{$windows}','{$oem_warranty}','{$sr_no_by_rac}','{$remarks}' ";
        $query .=")";
        if($db->query($query)){
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <div class="form-group">
            ...
            <div class="form-group">
                <label for="sr_no_by_rac">Sr. no. by RAC</label>
                <input type="text" class="form-control" name="sr_no_by_rac" placeholder="sr_no_by_rac">
            </div>
            <div class="form-group">
